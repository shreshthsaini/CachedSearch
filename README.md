...
The clips below are demonstrations, generated by running the procedure exactly as
described above. The paper's measured numbers are on Wan2.1-1.3B. A gallery of 104
...
Wan2.1-T2V-14B. The cached draft costs 332 s against 743 s, 45% of the price, and is
what the verifier ranks; the content it ranks on is preserved. On this pair the
...
### Calibrated on the models in the gallery

The paper's grid and the project-page gallery are different checkpoints on different
prompts, so every backbone in the gallery was recalibrated from its own rollouts rather
than inheriting a threshold. The rule is the paper's: take the largest tau whose gain
capture still clears 85%.

| Model | Family | Resolution | tau | Gain retained | Winner agreement | Exploration speedup |
|---|---|---|---:|---:|---:|---:|
| LTX-2 19B | LTX | 1920x1088 | 0.02 | 85.0% | 59% | 1.64x |
| Wan2.1-T2V-14B | Wan | 1280x720 | 0.10 | 85.9% | 56% | 2.23x |
| HunyuanVideo-13B | HunyuanVideo | 1280x720 | 0.05 | 94.8% | 65% | 1.77x |
| CogVideoX-5B | CogVideoX | 720x480 | 0.02 | 86.9% | 61% | 1.16x |
| Wan2.1-T2V-1.3B | Wan | 832x480 | 0.20 | 90.0% | 67% | 2.28x |

Measured on 222 prompts x 8 seeds x 2 arms with `code/experiments/calibrate_tau.py`.
Two thresholds do not transfer. LTX-2 driven at the Wan value collapses to 50.5%
retained on identical rollouts. CogVideoX at the paper's 0.05 retains only 71.4% on
these prompts and 81.2% at 0.03, so it runs at 0.02 and buys a smaller, honest saving.
