"""Training-free caching wrappers for video DiTs (Wan, CogVideoX, HunyuanVideo, LTX via diffusers).
...
class CacheHandle:
    """Handle over every wrapped transformer of a pipeline (Wan 2.2 has two)."""

    def __init__(self, wrapped: list[CachedTransformer]):
    ...
    def stats(self):
        return {f"transformer{i}": w.stats() for i, w in enumerate(self.wrapped)}

    @property
    def cfg(self) -> CacheConfig:
        return self.wrapped[0].cfg


def wrap_pipeline(pipe, cfg: CacheConfig) -> CacheHandle:
    """Attach caching to a diffusers video pipeline. Call .reset() between generations.

    Wraps ``pipe.transformer`` and, when present, ``pipe.transformer_2`` (Wan 2.2).
    Calling it again with a new config reconfigures the existing wrappers in place.
    """
    wrapped = []
    for name in ("transformer", "transformer_2"):
        ...
        if model is None:
            continue
        if isinstance(model, CachedTransformer):
            model.cfg = cfg
            model.reset()
        else:
            model = CachedTransformer(model, cfg)
            setattr(pipe, name, model)
        wrapped.append(model)
    if not wrapped:
        raise AttributeError("The pipeline exposes no `transformer` to wrap.")
    return CacheHandle(wrapped)
